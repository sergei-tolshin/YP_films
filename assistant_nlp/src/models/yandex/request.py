...
import attr
from core.utils.serialization import (FreeSerializeable, dict_converter,
                                      list_converter)
...
class NLU(FreeSerializeable):
    ...
    entities: list[Entity] = attr.ib(
        converter=list_converter(Entity), factory=list)
    intents: dict[str, Intent] = attr.ib(
        converter=dict_converter(Intent), factory=dict)
...
class Session(FreeSerializeable):
    ...
    location: Optional[Location] = attr.ib(
        default=None, converter=Location.from_dict)
