...
import attr
from core.utils.serialization import Serializeable, list_converter
...
class ItemsListFooter(Serializeable):
    text: str = attr.ib()
    button: Optional[CardButton] = attr.ib(
        default=None, converter=CardButton.from_dict)
...
class ItemsList(Card):
    ...
    header: Optional[ItemsListHeader] = attr.ib(
        converter=ItemsListHeader.from_dict, default=None)
    items: list[ItemsListItem] = attr.ib(
        converter=list_converter(ItemsListItem), factory=list)
    footer: Optional[ItemsListFooter] = attr.ib(
        converter=ItemsListFooter.from_dict, default=None)
...
class ImageGallery(Card):
    ...
    items: list[ImageGalleryItem] = attr.ib(
        converter=list_converter(ImageGalleryItem), factory=list)
...
class ShowItemMeta(Serializeable):
    ...
    publication_date: Optional[str] = attr.ib(default=None)
...
class Response(Serializeable):
    ...
    buttons: list[Button] = attr.ib(
        converter=list_converter(Button), factory=list)
    card: Optional[Union[BigImage, ItemsList, ImageGallery]
                   ] = attr.ib(converter=card_converter, default=None)
    end_session: bool = attr.ib(default=False)
    show_item_meta: Optional[ShowItemMeta] = attr.ib(
        default=None, converter=ShowItemMeta.from_dict)
